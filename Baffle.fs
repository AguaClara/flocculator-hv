...
export const bafflePreDesigner = function(design) returns map
    {
        ...
        design.pipe.fitting = queryFittingDim(design.pipe.ND, design.ip, FittingType.CAP, FittingShape.CAP, ConnectionStyle.SOCKET_CONNECT, ["OD", "socketL"])
        
        design.pipe.fitting.R = design.pipe.fitting.OD / 2; //distance from node to outer cap, TBD
        ...
        design.spacer.topbackL = design.baffleS*2 - design.pipe.fitting.socketL; //length of top back spacers
        design.spacer.botbackL = design.baffleS - design.pipe.fitting.socketL; //length of bottom back spacers
        design.spacer.front1D = design.channelL - 2*design.baffleB - design.pipe.fitting.socketL;
