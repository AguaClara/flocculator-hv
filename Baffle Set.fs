...
baffleModule::import(path : "c2f4584cf9d8b1114f7ff5b4", version : "e080987c0d4850b5fdcce4b2");
...
export const baffleSetGeometry = function(context is Context, id is Id, design is map) returns map
    {
        for (var i = 1; i < design.channelN; i += 1) //for every discrete number from 1 to N
        {
            ...
            {
               design.originY = -design.channelL / meter;
               //do not rotate
            }
        ...
        opMateConnector(context, id1, { 'coordSystem' : qlocation });
        const mateQ = qCreatedBy(id1, EntityType.VERTEX);

        superDerive(context, id1, {
